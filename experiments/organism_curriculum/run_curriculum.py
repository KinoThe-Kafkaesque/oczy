...
_CORRECTION_BENCHMARK_SRC = _REPO_ROOT / "correction-benchmark" / "src"
if str(_CORRECTION_BENCHMARK_SRC) not in sys.path:
    sys.path.insert(0, str(_CORRECTION_BENCHMARK_SRC))
...
for name in _ORGAN_SRC_DIRS:
    src = _REPO_ROOT / name / "src"
    if str(src) not in sys.path:
        sys.path.insert(0, str(src))

from experiments.organism import LMBackendAgent, OrganismAgent
from experiments.organism_curriculum.dataset import Episode, Probe, Stage, build_curriculum
from experiments.organism_curriculum.scoring import categorize_results, probe_matches
from experiments.organism_curriculum.validation import validate_curriculum
