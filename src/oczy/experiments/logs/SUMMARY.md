...
- CortexAgent's policy head can now optionally consume a request-context hidden vector
  ...
  modes. Stages 0+1 reach near-perfect retention/transfer with the real LM driver,
  and Stage 2 scope control is starting to show policy-driven alternate-sense selection.
44. (Run #91, no code change) — Directly compared `same-lm` and
    `foreign-minilm` embedders in the real-driver multi-fact stressor with a
    reserved-position prefix. Both reach `co_recall=1/1`; same-lm takes 8.8s,
    foreign-minilm takes 25.0s. Foreign-MiniLM preserves recall quality but
    is not faster in this regime. An attempt to add an ONNX Runtime foreign
    embedder failed because `optimum[onnxruntime]` requires
    `huggingface-hub<1.0`, which conflicts with the repo's `lm` dependency
    group (`>=1.0`); the environment was cleaned up and the fast suite passes.
    Benchmark `code_qa_accuracy=1.0` (run #91).
...
- CortexAgent's policy head can now optionally consume a request-context hidden vector
  ...
  modes. Stages 0+1 reach near-perfect retention/transfer with the real LM driver,
  and Stage 2 scope control is starting to show policy-driven alternate-sense selection.
45. `a2814e7` — Add `--auto-consolidate` mode to `multi_fact_stressor.py`.
    When active the probe lowers the consolidation-pressure threshold so
    `should_consolidate()` can fire after a single high-drift 512-token
    turn. Both scalar and hybrid modes auto-consolidate and hit the
    strength cap of 10.0, producing identical `cold_drift=0.867` and
    `co_recall=0/0` (cvec-only exact recall remains blocked). Fast suite
    `301 passed`; benchmark `code_qa_accuracy=1.0` (run #92).

Test status: `pytest: 301 passed` fast + 1 slow/real-driver construction test
(reserve-position + tensor-critic + replay-SGD + identity-adapter + hidden-delta +
default-critic + critic-gate + cortex-answer-loop + value-head + value-head-wiring +
policy-head + organism-policy + policy-correction-loop + policy-positive-reward +
actor-critic-baseline + acceptance-reward + curriculum-shim-margin +
...
ingestion-pipeline + needle-stressor + needle-sweep + salience-threshold +
mock-foreign-embedder + hybrid-consolidation + multi-fact-stressor +
foreign-minilm-embedder + real-driver-needle-sweep + auto-consolidate unit tests
pass; slow needle tests 4 passed, 1 slow real-driver construction test passes).
`ruff check` clean on changed files.

Remaining blocks:
- Architecture S vs H has not been behaviorally discriminated. With auto-consolidate
  both modes fire and hit the 10.0 strength cap. To see a difference, either remove
  the cap, use multi-turn pressure accumulation, or measure domain-level (not exact)
  recall in the multi-fact probe.
- ONNX-Runtime foreign embedder is blocked by `optimum`'s dependency range
  (`huggingface-hub<1.0`) conflicting with the repo's `lm` group.
- Direct reserved KV-slot injection still blocked by `llama-cpp-python` C API surface.
- Exact-token uptake via cvec alone remains blocked; `ReservedPosition` prefix is the
...
- CortexAgent's policy head can now optionally consume a request-context hidden vector
  ...
  modes. Stages 0+1 reach near-perfect retention/transfer with the real LM driver,
  and Stage 2 scope control is starting to show policy-driven alternate-sense selection.
47. `1c6572a` — Add `memory_bytes` reporting and `--max-traces` pruning to
    `multi_fact_stressor.py`. Under memory pressure (`--max-traces 1` and
    `2`), scalar and hybrid show identical `memory_bytes` and identical
    `co_recall`: 0/0 without prefix and 1/1 with prefix. Hybrid's ~3.6x higher
    consolidation strength does not reduce memory or improve recall in these
    probes. Added tests for `memory_bytes` and `max-traces`. Fast suite
    `304 passed`; benchmark `code_qa_accuracy=1.0` (run #94).

Test status: `pytest: 304 passed` fast + 1 slow/real-driver construction test
(reserve-position + tensor-critic + replay-SGD + identity-adapter + hidden-delta +
default-critic + critic-gate + cortex-answer-loop + value-head + value-head-wiring +
policy-head + organism-policy + policy-correction-loop + policy-positive-reward +
actor-critic-baseline + acceptance-reward + curriculum-shim-margin +
...
mock-foreign-embedder + hybrid-consolidation + multi-fact-stressor +
foreign-minilm-embedder + real-driver-needle-sweep + auto-consolidate +
hybrid-cap + memory-bytes + max-traces unit tests pass; slow needle tests 4
passed, 1 slow real-driver construction test passes).
`ruff check` clean on changed files.

Remaining blocks:
- The ingestion-design-matrix experiments are provisionally complete. The scaffold is
  instrumented and validated; further progress on behavior_delta_per_byte requires
  moving beyond exact-token recall probes and toward either hippocampus-derived
  reserved-position prefixes or a domain-level recall metric.
- Architecture S vs H shows a mechanical strength difference only when hybrid is
  uncapped; the extra strength does not improve recall or reduce memory.
- ONNX-Runtime foreign embedder is blocked by `optimum`'s dependency range
  (`huggingface-hub<1.0`) conflicting with the repo's `lm` group.
- Direct reserved KV-slot injection still blocked by `llama-cpp-python` C API surface.
- Exact-token uptake via cvec alone remains blocked; `ReservedPosition` prefix is the
...
- CortexAgent's policy head can now optionally consume a request-context hidden vector
  ...
  modes. Stages 0+1 reach near-perfect retention/transfer with the real LM driver,
  and Stage 2 scope control is starting to show policy-driven alternate-sense selection.
49. `f1f1392` — Add `--auto-prefix` to `multi_fact_stressor.py`: derive a
    `ReservedPosition` prefix from hippocampal traces/slow-updates instead of
    using the hand-coded `FACT_A + FACT_B` literal. The initial whole-utterance
    fallback returned mostly filler and scored `co_recall=0/0`; the final
    implementation extracts fact-bearing snippets around project-name keywords
    (`skylark`, `rook`, `alpha`, `beta`). Real-driver scalar and hybrid both
    reach `co_recall=1/1` at length 512 with `prefix_source=hippocampus`,
...
51. `e580526` — Add `--use-agent-prefix` to `multi_fact_stressor.py`. This
    validates the live `CortexAgent.use_hippocampus_prefix` path: the stressor
    enables the config flag, passes `recall_query` through `_recall_fact`, and
    captures the source of the `ReservedPosition` set by the agent during
...
52. `c6a5beb` — Deprecate `--auto-prefix` in `multi_fact_stressor.py`. The
    ...
    (run #99).
53. `3fed719` — Add `prefix_targets` support to `CortexAgent.articulate()` and
    `_derive_reserved_position_from_hippocampus()`. Expected answer strings are
    added to the snippet-extraction keyword set, so exact target tokens can be
    surfaced even when the recall query is paraphrased. `multi_fact_stressor` now
    passes the expected target via `prefix_targets`. Added a mock test verifying
    the target token appears in the derived prefix when the query omits it.
...
- CortexAgent's policy head can now optionally consume a request-context hidden vector
  ...
  `use_policy_request_context`. The ranking contribution remains normalized to
  softmax probabilities, and policy updates fire on every correction. The
  `policy_suppresses_fast_answer` flag lets the head head dominate final ranking in probe
  modes. Stages 0+1 reach near-perfect retention/transfer with the real LM driver,
  and Stage 2 scope control is starting to show policy-driven alternate-sense selection.
- CortexAgent now has a configurable `IngestionPipeline` upstream of metabolism with
