...
operates on structured Episodes per ``oczy_common.episode.py``).  The LM
...
from oczy_common.episode import EPISODE_FIELDS, validate_episode
...
class LanguageAdapter:
    ...
    def load(self) -> None:
        ...
        if self._loaded:
            return
        # Import inside the method so ``import oczy_lm`` stays cheap and
    ...
    def nl_to_episode(self, text: str) -> dict[str, Any]:
        ...
        Returns:
            A dict conforming to :class:`oczy_common.episode.Episode`.
        """
        self.load()
