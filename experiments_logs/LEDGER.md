...
| 2026-07-19 | `2026-07-16_campaign_959e114.md` | VALID | **R20 INT8 meta_cortex/v2 DEV training and calibration campaign.** Curated evidence log: five exact-config checkpoints are verified; frozen instrument `f62f18ef…` and runtime manifest `a6214355…` remain unchanged. The original 90-job calibration batch was infrastructure-invalid because Kaggle auto-extracted registered `.tar.gz` inputs. Commit `949871b3d2d6cbb8a68af41396639cfb3f79ee5e` cut transport over to hash-verified opaque `.tar.gz.bin` files and content-bound publication resume state (200 focused tests passed). A corrected five-task canary verified archive/runtime loading but hit the 12h CPU limit. The v5 one-task canary succeeded in 2.853h and passed source, CPU, runtime, organ, shard-header, and shard-self-hash gates; a complete 116-job bounded partition then entered five-wide Kaggle fanout. Live at 2026-07-19 20:02 UTC: 5 succeeded, 1 infrastructure-failed at 12.010h, 5 running, 105 pending. Four-task shards have insufficient deterministic provider margin; the failed attempt is preserved and requires a narrower uniquely named replacement. No scientific verdict; DEV collection is incomplete and meta-test remains blocked/unaccessed. | — |
