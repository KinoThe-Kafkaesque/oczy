...
Completion: C1 done, C2 partial-but-tracked, C3 tested, C4 untested =>
0.875. If the GGUF driver is unavailable the C3 measurement cannot run and
the lane falls back to 0.75 (no regression from the prior constant). On any
import/load/crash failure, measure() returns float('nan').
"""
...
def measure() -> float:
    try:
        ...
        # exercised end-to-end on the real driver -> 0.875 (C1 done, C2
        # partial, C3 tested, C4 untested). delta is computed but does not
        # gate the status; even a negative measured delta counts as "tested".
