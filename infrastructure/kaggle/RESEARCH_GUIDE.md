...
### 8. Parallel batch scheduling (advanced)

For fan-out over multiple seeds, configurations, or ablation conditions, use
the **parallel scheduler** instead of submitting kernels one by one. It
manages a batch of private CPU-only kernels with bounded concurrency and
crash-safe resume.

**Batch manifest** (``oczy/kaggle-parallel-batch/v1``):
...
```json
{
  ...
  "jobs": [
    {
      "name": "dev-seed-0",
      "kernel_dir": "build/dev-seed-0",
      "output_dir": "reports/kaggle/dev-seed-0"
    },
    {
      "name": "dev-seed-1",
      "kernel_dir": "build/dev-seed-1",
      "output_dir": "reports/kaggle/dev-seed-1"
    }
  ]
}
```

Each ``kernel_dir`` must be a kernel directory generated by step 4
(``prepare_research_kernel.py``). Paths in the manifest resolve relative
to the manifest file's directory. The scheduler validates every kernel
before any submission and rejects:

- non-private or non-CPU kernels;
- title/slug mismatches (the title-derived Kaggle URL slug must match the
  kernel id slug — a real failure discovered during scheduler development);
- duplicate names or kernel ids.

**Generate kernel directories for each seed**, for example:

```bash
for seed in 0 1 2 3; do
  JOB="infrastructure/kaggle/build/dev-seed-${seed}"

  uv run python infrastructure/kaggle/prepare_research_kernel.py \
    --output "$JOB" \
    --kernel-id "abdellahkadem/oczy-meta-cortex-dev-${seed}" \
    --title "Oczy Meta Cortex Dev ${seed}" \
    --phase development \
    --profile cpu \
    --source-dataset "$SOURCE_DATASET" \
    --source-commit "$SOURCE_COMMIT" \
...
```bash
# Run the batch (default 8 concurrent, max 10)
uv run python infrastructure/kaggle/parallel_scheduler.py run \
  infrastructure/kaggle/build/dev-batch.json \
...
**Lifecycle.** The durable state file is written atomically after every
transition. States flow: ``pending -> submitting -> running -> collecting ->
succeeded``, with terminal ``failed`` on error or timeout. Resuming a
stopped process re-runs the same command: interrupted ``submitting``
reverts to ``pending``, interrupted ``collecting`` reverts to ``running``,
and already-running kernels are never resubmitted. Only new jobs from an
updated manifest are added on resume.

**Concurrency bounds.** The default is 8 concurrent remote jobs. The
maximum is 10. Raise ``--max-parallel`` after checking ``kaggle quota``.
The scheduler is single-threaded; parallelism means N remote Kaggle slots,
not N local threads.

**Title/slug mismatch guard.** The scheduler rejects any kernel whose
title-derived Kaggle clean-URL slug differs from the kernel id's final
path component. This prevents the real failure mode where a kernel with
id ``abdellahkadem/oczy-scheduler-smoke-1`` and title ``Oczy Scheduler CPU
Smoke 1`` is created under slug ``oczy-scheduler-cpu-smoke-1``, making
polling fail.
